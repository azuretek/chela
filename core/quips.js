...
//
// They are jokes and they are labelled as such, none of them claims to describe
// a step the app is performing. That is deliberate: a fake status message
// ("Verifying certificates...") next to a real progress bar is a lie told in the
// one place someone is looking for a reason their app will not open. The real
// state is the bar, the gateway URL above it, and the banner when it fails.
//
// Pure and platform-free: the rotation is an index into a frozen list, so the
// order and the wrap are testable without a window. The list and the rotation
// interval are data, read from spec/quips.json so a Swift port shares the exact
// same source of truth.

import spec from './spec/quips.json' with { type: 'json' };
 ...
 */
export const QUIPS = Object.freeze([...spec.quips]);

/** How long each line stays up. Long enough to read twice, short enough to move. */
export const ROTATE_MS = spec.rotateMs;
 ...
 */
export function quipAt(step, offset = 0) {
 ...
 * assert the sequence.
 */
export function startAt(seed = Math.random()) {
