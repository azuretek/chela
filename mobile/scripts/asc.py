...
Reads only what the release needs, and changes nothing: it cannot create or
revoke a certificate, a profile or an app record, and the 403 Apple returns for
any of those is why the signing certificate is imported from a stored .p12 in
the workflow rather than minted here. The key id and issuer id are not secret;
the key itself is.
"""
