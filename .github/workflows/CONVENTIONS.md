...
- **Every job the other pipeline lists**, every one of which must conclude `success`. Only `success` passes. A cancelled or timed-out leg is not a passing one, and a **skipped** one is not evidence either: the mobile `release` job is the case that matters, and a skipped upload means no installable iOS build exists. The required set is the other run's WHOLE job list, read with no pattern over it, because a pattern is a promise that it names everything and a job added, renamed or re-scoped in that pipeline falls outside the promise silently.
- **A run's job list lags the run itself.** A run object appears within seconds of its push while its jobs register seconds to tens of seconds later, so the gate waits for jobs to appear rather than reading an empty list as a rename, and refuses on an empty list only once the other run has concluded without ever listing one. Reading the empty list as a failure is what refused a release whose counterpart was green on 2026-09-17.
- **All of them, not a named subset.** Abi's rule of 2026-09-21: "require all jobs to finish successfully before we publish, so that we can never have something claiming to be deployed and not." The gate used to require a **verdict**, a hand-written alternation of job-name patterns, and the class that closes is a job the pattern does not name: for as long as nobody remembered to add it, it was outside the gate, and a pattern that names everything looks exactly like one that does not. Measured on 2026-09-21, the mobile run for `c8d9db5675` had `iOS 26`, `iOS 27` and `swiftlint` concluded `success` while its `release` job (the TestFlight upload and the wait for VALID) had not registered at all, so the desktop published unmarked and no phone could be offered an update from it. There is no pattern any more: the gate reads that run's whole job list, so a job added to a pipeline is inside the gate the moment it exists.
- **Not the jobs that cannot conclude before this pipeline publishes.** Each release job needs its own pipeline's gate, and each gate waits on the other pipeline, so waiting on the other publish (and, for the desktop gate, on the desktop's own call of the gate) would deadlock both runs. Those are the only exemptions, named per call site in the gate's `exempt` input; nothing else is outside the requirement. The upload is not a build or a test either, which is why the phone's upload is required and only the desktop's publish is not.
...
**★ The gate's own shell is checked by a test.** A gate that cannot run refuses everything, so `desktop/test/release-trigger.test.js` parses every `run: |` block of these three workflows with `bash -n`. Measured 2026-09-21: a change to this file merged on green CI while its `run` block did not parse, `bash` died on a syntax error a second into the gate job, both gates refused, and NOTHING published from `main` while every workflow's own checks reported success. A guard that matches strings cannot catch that; a parser can, and it needs no credentials, no network and no side effects.
...
**The two readers are kept in agreement by the parity fixture** (`core/fixtures/feed.json`'s `iosCases`, asserted by `core/test/feed.test.js` and `mobile/ChelaTests/UpdateFeedParityTests.swift`): a release without the marker is filtered out by the iOS reader, one with it is offered, and the desktop reader is proven to still read every release.
