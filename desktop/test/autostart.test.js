...
test('disabling removes it rather than writing a disabled one', () => {
  const fs = fakeFs();
  const r = autostart.apply({ enabled: false, fs, env: {}, home: '/home/example-user' });
  assert.equal(r.ok, true);
  assert.equal(r.wrote, false);
  assert.equal(fs.files.size, 0);
  assert.deepEqual(fs.removed, [autostart.entryPath({ env: {}, home: '/home/example-user' })]);
});
...
test('a write failure is reported, not swallowed', () => {
  ...
  const r = autostart.apply({ enabled: true, fs, env: {}, home: '/home/example-user' });
...
test('removing an entry that was never there is not a failure', () => {
  ...
  // it was already off.
  assert.equal(autostart.apply({ enabled: false, fs, env: {}, home: '/home/example-user' }).ok, false);
});
