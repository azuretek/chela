...
const build = (platform) => menus.template({ platform, appName: 'Chela', commands: commands() });
...
test('only macOS has an application menu, and it comes first', () => {
  ...
  assert.equal(mac[0].label, 'Chela', 'macOS turns the first menu into the application menu');
...
test('a missing command is a build error, not a silently absent menu item', () => {
  ...
  delete short.about;
  assert.throws(
    () => menus.template({ platform: 'win32', appName: 'Chela', commands: short }),
    /no command "about"/,
  );
});
